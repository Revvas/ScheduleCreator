...
void TeachersWidget::searchClosed(QString filter){
    comboGroup->setCurrentText(filter);
    currentGroupChanged(filter);
}

void TeachersWidget::Reload(){
    QString text = comboGroup->currentText();
    for(int i=0; i<6; i++){
        ModelAr[i]->submitAll();//ModelAr[i]

        if(comboWeek->currentText() == "Нижняя")
           ModelAr[i]->setFilter(QString("Teacher = '%1' AND UpDown = '0' AND Day ='%2'").arg(text).arg(QsAr[i]));
        else
...
void TeachersWidget::searchClicked(bool checked){
    QSqlTableModel *searchModel = new QSqlTableModel(0, _database);
    searchModel->setTable("Teachers");//GroupMo
    searchModel->select();
...
//////////////////////////////////
    ...
    teacherWidget->setLayout(qll);
    teacherWidget->resize(700, 500);
...
void TeachersWidget::AddTable(QGridLayout *qll, QSqlDatabase _database){
...
void TeachersWidget::on_addButton_click(){
    for(int i=0; i<6; i++){
...
void TeachersWidget::currentGroupChanged(const QString &text){
    TableChanged();
    for(int i=0; i<6; i++){
        ModelAr[i]->submitAll();//ModelAr[i]
...
void TeachersWidget::currentWeakChanged(const QString &text){
   currentGroupChanged(comboGroup->currentText());
}
